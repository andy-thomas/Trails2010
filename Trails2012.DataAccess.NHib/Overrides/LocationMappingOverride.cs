...
namespace Trails2012.DataAccess.NHib.Overrides
{
    public class LocationMappingOverride : IAutoMappingOverride<Location>
    {
        public void Override(AutoMapping<Location> mapping)
        {
            ...
            // which occurs due to NHib trying to map RegionId twice - 
            //          once through RegionId property, and once through the relationship to the Region object.
            // (I guess I could lose the RegionId property altogether and use Region.Id, but that would break my implementation 
            // of the Entity Framework repostitory plug-in - why query the related tables if you do not need to? The foreign key is all 
            // that's needed if using drop down data-entry controls)
